...
// When the `wee_alloc` features is enabled, use `web_alloc` as the global
// allocator
...
* ディスクのベクタを初期化する
*/
fn init_disks(disk_num: u32, bound_x: u32, bound_y: u32) -> Vec<Disk> {
    let mut disks_buffer: Vec<Disk> = Vec::with_capacity(disk_num as usize);

    let mut rand = rand::thread_rng();
    for i in 0..disk_num {
        let random = rand.gen_range(0f64..1f64);
        let velocity = 1f64 + 3f64 * random;
...
impl Screen {
     ...
     */
    fn draw(&self) {
        self.gl.clear_color(0f32, 0f32, 0f32, 1f32);
        ...
        self.gl
            .vertex_attrib3f(self.attrib_color as u32, 1f32, 0f32, 0f32);
        self.gl
...
pub fn init_gl(option_input: JsValue) -> Screen {
    let options: Options = serde_wasm_bindgen::from_value(option_input).unwrap();
    ...
    let disk_size = options.disk_size.unwrap_or(32f64);
    ...
    // ランダム生成した浮動小数点値を1disk当たりに3値（rgb）割り当てる
    let mut random = rand::thread_rng();
    let color_buffer_array = (0..(disk_num * 3))
        .map(|_| random.gen_range(0f32..1f32))
        .collect::<Vec<f32>>();
